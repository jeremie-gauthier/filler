...
/*   Updated: 2020/07/08 11:46:17 by jergauth         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				play_piece(t_filler *filler)
{
	...
	if (pawns_init(&filler->me->pawns,
					filler->map->height * filler->map->width) < 0)
		return (-1);
	pawns_counting(filler);
	pawns_sorting(filler->me->pawns, filler->me->len_pawns);
	pawns_iter_coords(filler);
	if (!(best_pawn = pawns_select_best(
						filler->me->pawns, filler->me->len_pawns)))
		ft_printf("0 0\n");
	else
		ft_printf("%i %i\n",
