...
/*   Updated: 2020/07/08 11:45:07 by jergauth         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static unsigned int	find_min_dist(t_filler *filler, const t_coord coord)
{
	...
	if (coord.y + 1 < filler->map->height
			&& heatmap[coord.y + 1][coord.x] < min)
		min = heatmap[coord.y + 1][coord.x];
...
static int			pawns_add(t_filler *filler, const t_coord coord,
								const size_t idx)
{
	t_pawn	pawn;
